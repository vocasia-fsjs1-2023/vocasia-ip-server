...
class Controller {
  static async createOrder(req, res) {
    const body = req.body;
    const getUserId = req.userId; //Get from middleware authentication/JWT and passing to next middleware and last to controller
    ...
    try {
      ...
      res.status(201).json({ message: "Berhasil menambahkan order!", Result });
    } catch (error) {
      console.log(`Error menambahkan order! ${error}`);
  ...
  static async updateOrder(req, res) {
    ...
    try {
      ...
      console.log(`Error mengupdate order! ${error}`);
      res.status(500).json(error);
    }
  }
  ...
  static async deleteOrder(req, res) {
    ...
    try {
      ...
      console.log(`Error menghapus order! ${error}`);
      res.status(500).json(error);
    }
  }
}
